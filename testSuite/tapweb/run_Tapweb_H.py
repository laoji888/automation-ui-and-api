...
if __name__ == "__main__":
    casepath = path.TEST_CASE_DIR + "/tapweb/"
    # 加载测试场景
    test_suite = unittest.defaultTestLoader.discover(casepath, pattern='test*.py')

    # 运行测试场景
    result = BeautifulReport(test_suite)
    result.report(filename=currentime + '规划管理-自动化测试报告', description='规划管理-自动化测试报告', log_path=path.TEST_REPORT)
    case_count = dircount(casepath)
    # 将测试报告发送至邮箱，收件人，有多个时用逗号隔开
    send_test_report("规划管理-UI自动化测试报告", case_count)
